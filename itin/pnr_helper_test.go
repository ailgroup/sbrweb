...
var (
	...
	serverPsngrDetails = &httptest.Server{}
)

//Initialize Mock Sabre Web Servers and test data
func init() {
	// init test servers...
	serverPsngrDetails = httptest.NewServer(
		http.HandlerFunc(
			func(rs http.ResponseWriter, rq *http.Request) {
				rs.Write(samplePsngrRes)
			},
		),
	)
	//defer func() { serverPsngrDetails.Close() }()

	serverBizLogic = httptest.NewServer(
		http.HandlerFunc(
			func(rs http.ResponseWriter, rq *http.Request) {
				rs.Write(samplePsngrResWarnBizLogic)
			},
		),
	)
...
var (
	...
	samplePsngrResWarnBizLogic = []byte(`<?xml version="1.0" encoding="UTF-8"?>
	...
	samplePsngrRes = []byte(`<?xml version="1.0" encoding="UTF-8"?>
